...
@Controller(`${COIN_URL}/:id/verify`)
export class CoinVerifyController extends DefaultController<number, Coin> {
    // --------------------------------------------------------------------------
    //
    //  Constructor
    //
    ...
    @Swagger({ name: 'Coin verify', response: Coin })
    @Post()
    @OpenIdNeedResources()
    ...
    public async executeExtended(@Param('id', ParseIntPipe) id: number, @OpenIdBearer() bearer: IOpenIdBearer): Promise<ICoinEditDtoResponse> {
        ...
        CoinUtil.isCanVerify(item, bearer.resources, true);
        return this.transport.sendListen(new CoinEditCommand({ id, status: CoinStatus.VERIFIED }));
    }
}
